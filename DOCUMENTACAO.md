...
- **Breakpoints Responsivos Validados:**
  ...
  - `Mobile / Smartphones (<= 640px e <= 480px)`: Ajuste automático de fontes com `clamp()`, layout em coluna única, alinhamento justificado à esquerda para botões de filtros e abas (`.gallery-filters`, `.events-filter-container`, `.menu-tabs-container`), redimensionamento proporcional e compacto do botão "Reservar VIP" no header (`.header-actions .btn`) para harmonia estética com o logotipo e botão de menu hamburguer, botão flutuante **Lista VIP 20% OFF** (`.floating-vip-btn`) posicionado no canto inferior esquerdo (`left: 14px / 12px / 8px`), com alto contraste, coroa dourada brilhante, animação pulsante suave `@keyframes vipFloatPulse`, limites de largura seguros `max-width: calc(100vw - 24px)` sem sobrepor o conteúdo ou ser cortado na borda da tela, modal de Lista VIP (`#vip-popup-modal` / `.vip-popup-card`) 100% responsivo e centralizado com paddings otimizados, formulários e botão de ação perfeitamente enquadrados e rolagem vertical suave sem cortes horizontais, modais em tela cheia com rolagem nativa e eliminação completa de scroll horizontal.
